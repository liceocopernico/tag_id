...
class Tag:
    ...
    def __post_init__(self):
        current_tag:tag=self.dataStore.get_tag(self.tag_id)
...
class Roster:
    ...
    def __post_init__(self):
        ...
        if self.code:
            self.roster_data=self.dataStore.get_roster(self.code)
            self.name=self.roster_data.name
            self.tags=[Tag(tag_id=data.tag.tag_id,ord=idx) for idx,data in enumerate(self.dataStore.get_roster_tags(self.code))]

        else:
            self.name=datetime.now().strftime("%d/%m/%Y, %H:%M:%S")
            self.roster_data=self.dataStore.add_roster(self.name)
...
class Rosters:
    ...
    def __post_init__(self):
        self.past_rosters=self.dataStore.get_rosters()
